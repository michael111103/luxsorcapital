...
function StatCard({ item }: { item: StatItem }) {
  ...
  return (
    <div ref={ref} className="group relative flex flex-col items-center">
      {/* half‑box header that grows on hover */}
      ...
      {/* only render count once in view */}
      {inView && (
      ...
      <section className="px-6 md:px-16 py-16 bg-black" id="features">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {features.map((f) => (
            <div
              key={f.title}
              className="flex flex-col h-full bg-zinc-900/60 rounded-xl p-6 border border-zinc-800 hover:ring-1 hover:ring-white/20 transition"
            >
              <Image
                src={f.img} alt={f.title}
                width={400} height={240}
                className="rounded-lg w-full h-auto mb-4 flex-shrink-0"
              />
              <h3 className="text-xl font-semibold mb-2 text-white">{f.title}</h3>
              <p className="text-sm text-white/70 flex-1">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
